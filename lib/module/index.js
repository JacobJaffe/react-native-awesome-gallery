...
import { I18nManager, Image, StyleSheet, useWindowDimensions } from 'react-native';
import Animated, { useAnimatedStyle, useDerivedValue, useSharedValue, withTiming, withDecay, useAnimatedReaction, runOnJS, withSpring, cancelAnimation } from 'react-native-reanimated';
import { Gesture, GestureDetector, GestureHandlerRootView } from 'react-native-gesture-handler';
import { clamp, withDecaySpring, withRubberBandClamp, resizeImage } from './utils';
const rtl = I18nManager.isRTL;
const DOUBLE_TAP_SCALE = 3;
const MAX_SCALE = 6;
const SPACE_BETWEEN_IMAGES = 40;
export const useVector = (x1 = 0, y1) => {
  const x = useSharedValue(x1);
  const y = useSharedValue(y1 !== null && y1 !== void 0 ? y1 : x1);
  return {
    x,
    y
  };
...
const ResizableImage = /*#__PURE__*/React.memo(({
    ...
    if (scale.value < 1) {
      ...
      if (newWidth <= width) {
        ...
        if (lockVerticalPanUntilExceedsZoom) {
          ...
          const lockY = !!(isVertical !== null && isVertical !== void 0 && isVertical.value) && newHeight <= height;
  ...
  const panGesture = Gesture.Pan().enabled(swipeEnabled).minDistance(10).maxPointers(1).onBegin(() => {
    'worklet';
    ...
    const x = getEdgeX();
    const newHeight = scale.value * layout.y.value; // This can be a prop, or just always be how the logic is decided.

    const lockY = lockVerticalPanUntilExceedsZoom ? !isVertical.value && newHeight <= height : false;
    const lockX = lockVerticalPanUntilExceedsZoom ? isVertical.value && newHeight <= height : isVertical.value && scale.value === 1;
  ...
  return /*#__PURE__*/React.createElement(GestureDetector, {
    gesture: Gesture.Race(Gesture.Simultaneous(longPressGesture, Gesture.Race(panGesture, pinchGesture)), Gesture.Exclusive(doubleTapGesture, tapGesture))
